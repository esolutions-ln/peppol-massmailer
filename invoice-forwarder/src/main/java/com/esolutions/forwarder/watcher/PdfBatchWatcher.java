...
public class PdfBatchWatcher {
    ...
    private record Pending(Path pdf, Path sidecar, SidecarData data) {}
    ...
    public PdfBatchWatcher(ForwarderProperties props,
        ...
        this.fileHandler = new WatcherFileHandler(
                Path.of(props.processedDir()),
                Path.of(props.failedDir()),
                null
        );
    }
    ...
    private void ingest(Path pdf) {
        ...
        if (!awaitFile(sidecar)) {
            ...
            fileHandler.moveToFailed(pdf);
            failedCount.incrementAndGet();
            return;
        }
        try {
            SidecarData data = objectMapper.readValue(sidecar.toFile(), SidecarData.class);
            validate(data, base);
            ...
            fileHandler.moveToFailed(pdf);
            fileHandler.moveToFailed(sidecar);
            failedCount.incrementAndGet();
        }
    }
    ...
    private void validate(SidecarData s, String base) {
    ...
    void flushNow() {
        ...
        try {
            ...
            for (Pending p : batch) {
                fileHandler.moveToFailed(p.pdf());
                fileHandler.moveToFailed(p.sidecar());
                failedCount.incrementAndGet();
            }
        }
    }
    ...
    private void submitBatch(String batchId, List<Pending> batch) throws IOException {
        ...
        List<Pending> succeeded = new ArrayList<>();

        for (Pending p : batch) {
            SidecarData s = p.data();

            try {
                byte[] pdfBytes = Files.readAllBytes(p.pdf());

                if (!PdfValidator.isValidPdf(pdfBytes, p.pdf().getFileName().toString())) {
                    log.warn("Invalid PDF magic bytes for {} — moving to failed", p.pdf().getFileName());
                    fileHandler.moveToFailed(p.pdf());
                    fileHandler.moveToFailed(p.sidecar());
                    failedCount.incrementAndGet();
                    continue;
                ...
                String invoiceNumber = s.effectiveInvoiceNumber(p.pdf());
                ...
                log.error("Failed to process {} in batch {}: {}", p.pdf().getFileName(), batchId, e.getMessage());
                fileHandler.moveToFailed(p.pdf());
                fileHandler.moveToFailed(p.sidecar());
                failedCount.incrementAndGet();
            }
        ...
        if (entries.isEmpty()) {
            log.warn("Batch {} has no valid entries — skipping submission", batchId);
            return;
        }

        SidecarData first = batch.get(0).data();
        var req = new MailerDtos.CampaignRequest(
                first.effectiveCampaignName() + " (batch " + batchId + ")",
                first.effectiveSubject(),
                first.effectiveTemplateName(),
        ...
        for (Pending p : succeeded) {
            fileHandler.moveToProcessed(p.pdf());
            fileHandler.moveToProcessed(p.sidecar());
